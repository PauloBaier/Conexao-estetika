...
public class MovimentacaoCaixaService {
    ...
    public double calcularSaldo(Caixa caixa){
        ...
        for (MovimentacaoCaixa mov : lista) {
            ...
            if (!mov.getCaixa().getId().equals(caixa.getId())) {
                continue;
            }
    ...
    public MovimentacaoCaixa deletar(Long id){
        ...
        if (mov.getCaixa().getStatus() == StatusCaixa.FECHADO) {
            throw new IllegalArgumentException("Não pode alterar caixa fechado!");
        }
